...
@Component
class DataInitializer (val ingredientDAO: IngredientDAO, val caracteristiqueDAO: CaracteristiqueDAO,val mentionDAO: MentionDAO,val recetteDAO: RecetteDAO,val resultatDAO: ResultatDAO,val ligneIngredientDAO: LigneIngredientDAO) : CommandLineRunner {
    override fun run(vararg args: String?) {
        ...
        if (recetteDAO.count() == 0L) {
            ...
            var ligneIngId=LigneIngredientId(ingredientId = 2,2)
            var ligneIng=  LigneIngredient(quantite = 250.0f, pourcentage = 25.0f, ingredient = olive, recette = recetteDAO.findById(2).get(), ligneIngredientId = ligneIngId)
            ligneIngredientDAO.save(ligneIng)
